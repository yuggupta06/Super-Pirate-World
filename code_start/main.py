...
class Game:
    def __init__(self):
        ...
        self.current_stage=Level(self.tmx_maps[self.data.current_level],self.level_frames,self.audio_files,self.data,self.switch_stage,self.items_disappear)
    ...
    def switch_stage(self,target,unlock=0):
        if target=="level":
            if self.data.current_level!=0 and self.level_health_plus[self.data.current_level]==True:
                self.level_health_plus[self.data.current_level]=False
                self.data.health+=1
            self.current_stage=Level(self.tmx_maps[self.data.current_level],self.level_frames,self.audio_files,self.data,self.switch_stage,self.items_disappear)
